...
public class KafkaHealthMetricSyncTask implements Runnable{
    ...
    private static final LoggerMaker loggerMaker = new LoggerMaker(KafkaHealthMetricSyncTask.class);
    ...
    public void run() {
        try {
            loggerMaker.infoAndAddToDb("SYNCING");
            ...
            loggerMaker.infoAndAddToDb("SYNC DONE");
        } catch (Exception e) {
            loggerMaker.errorAndAddToDb("ERROR in kafka data sync from api runtime");
        }
    }
}
